...
def norad_to_name(norad_id):
	if type(norad_id) == type('some string'):
		if norad_id.isdigit():
			...
			return norad_id
	tlefile_lines=open('./API/DEV/tle.txt').read().split('\n')
...
def get_next_sat_coordinates(sat_name='QMR-KWT', scheduled_time=datetime.utcnow(), length=2, tolerance=.001, horizon=0):
	...
	sat=Orbital(sat_name, "./API/DEV/tle.txt")
...
@register.simple_tag(name="get_next_passes_info")
def get_next_passes_info(sat_name, lon=gelaLon,lat=gelaLat,alt=gelaLat,hours=72,horizon=15, tolerance=.001):
	...
	for p in passes:
		...
		info.append(((p[0]+timedelta(hours=3)).strftime("%d.%m."),(p[0]+timedelta(hours=3)).strftime("%H:%M"),(p[1]+timedelta(hours=3)).strftime("%H:%M"),round(apogee,1)))
	return info
